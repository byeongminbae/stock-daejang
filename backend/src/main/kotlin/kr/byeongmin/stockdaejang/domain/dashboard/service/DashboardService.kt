...
class DashboardService(
	...
	private fun getLatestMarketPriceDto(
		...
		return stockCodes
			.mapNotNull { marketPricesByStockCode[it] }
			.maxWithOrNull(
	...
	private fun getOwnerResponseDtosOrEmptyList(
		...
		return owners.map { owner ->
			...
			val dashboardBrokerageResponseDtos = ownedPositionsByBrokerageId.values
				.map { dashboardPositions ->
					val brokerageTotalBuyAmount = dashboardPositions.sumOfDecimal { it.totalBuyAmount }
					val dashboardStockResponseDtos = dashboardPositions.mapNotNull { dashboardPosition ->
						marketPricesByStockCode[dashboardPosition.stock.stockCode]?.let { marketPriceDto ->
					...
					DashboardBrokerageResponseDto.of(
						brokerage = dashboardPositions.first().brokerage, // brokerageId 그루핑이라 아무거나 선택해도 같음
